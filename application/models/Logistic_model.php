...
/*    public function Add_LoanData($data){
    ...
    public function Add_LogisticeData($data){
        return $this->db->insert('logistic_asset',$data);
    }
    public function Add_LogisticeSupport($data){
        return $this->db->insert('logistic_assign',$data);
    }
    public function Add_Assets_Category($data){
        return $this->db->insert('assets_category',$data);
    }
/*    public function LogisticValue(){
    ...
    public function Update_LogisticeData($id,$data){
       ...
       return $this->db->update('logistic_asset',$data);        
    }
    public function Update_LogisticeSupport($id,$data){
        $this->db->where('ass_id',$id);
       return $this->db->update('logistic_assign',$data);        
    }
    ...
    public function Update_Assets_Category($id,$data){
       ...
       return $this->db->update('assets_category',$data);
    } 
    public function GetAssetsValueId($id){
     ...
     public function AssetCatdelete($id)
    {
    // return $this->db->delete('assets_category',array('cat_id'=> $id));
        // Check if the category is referenced in the 'assets' table
        $this->db->where('catid', $id);
        $assetsCount = $this->db->count_all_results('assets');

        if ($assetsCount > 0) {
            // Category is referenced in 'assets' table, so don't delete
            return false;
        } else {
            // Category is not referenced, delete it from 'assets_category' table
            return $this->db->delete('assets_category', array('cat_id' => $id));
        }
    }
     public function Assetdelete($id)
    {
     return $this->db->delete('assets',array('ass_id'=> $id));
    }
