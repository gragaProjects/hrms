...
/*    public function View(){
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if(empty($id)){
              ...
              /* if($success){
                ...
                $succ = $this->logistic_model->Update_Assets($logid,$data);
                // echo "Successfully Updated";
                   if($succ){
                        echo json_encode(array('status'=>'success','message'=>'Successfully Added '));
                      }
    ...
    public function Add_Assets_Category(){
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $data = array('cat_name'=> $catname,'isActive'=> 1);
            if($this->organization_model->Check_field_exists($val,$data,$table)){
            echo json_encode(array('status'=>'error','message'=>'<p>This Assets Category  is already exists</p>'));
            } else {
            $data = array();
                $data = array(
                    'cat_name' => $catname,
...
if($this->session->userdata('user_login_access') != False) {         
    ...
    public function AssetCatdelete(){
        if($this->session->userdata('user_login_access') != False) { 
            $id = $this->input->post('id');
            $result_del = $this->logistic_model->AssetCatdelete($id);//
          if($result_del){
                echo json_encode(array('status'=>'success','message'=> 'Deleted Successfully'));
               
            }else{
                 echo json_encode(array('status'=>'error','message'=> 'This category already used in assets'));
        ...
        if($this->session->userdata('user_login_access') != False) { 
            $id = $this->input->post('id');
            $result_del = $this->logistic_model->Assetdelete($id);//
          if($result_del){
                echo json_encode(array('status'=>'success','message'=> 'Deleted Successfully'));
