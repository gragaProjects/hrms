...
class Projects extends CI_Controller
{
	...
	public function Pending_Projects()
	{
		...
		/*	if ($this->session->userdata('user_type') == 'EMPLOYEE') {
			...
			$data['projects'] = $this->project_model->PendingProjectsValue();

			$this->load->view('backend/pending_projects', $data);
		} else {
			redirect(base_url(), 'refresh');
		}
	}
	...
	public function Add_Projects()
	{
		if ($this->session->userdata('user_login_access') != False) {
			...
			$this->load->library('form_validation');
			$this->form_validation->set_error_delimiters();
			$this->form_validation->set_rules('protitle', 'project Title', 'trim|required|max_length[220]|xss_clean');
			$this->form_validation->set_rules('details', 'details', 'trim|max_length[1024]|xss_clean');
			...
			$val = $title;
            $table = 'project';
            $data = array('pro_name'=> $title,'isActive'=> 1);
            if($this->organization_model->Check_field_exists($val,$data,$table)){
            echo json_encode(array('status'=>'error','message'=>'<p>This Project  is already exists</p>'));
            } else {
				$data = array();
				$data = array(
					'pro_name' => $title,
	...
	public function Add_Tasks()
	{
		...
		/*	$this->load->library('form_validation');
			$this->form_validation->set_error_delimiters();
			$this->form_validation->set_rules('tasktitle', 'tasktitle', 'trim|required|max_length[150]|xss_clean');
		...
		/*	if ($this->form_validation->run() == FALSE) {
		   ...
		   $val = $title;
            $table = 'pro_task';
            $data = array('task_title'=> trim($title),'pro_id'=> $proid,'isActive'=> 1);
            if($this->organization_model->Check_field_exists($val,$data,$table)){
            echo json_encode(array('status'=>'error','message'=>'<p>This task  is already exists</p>'));
            } else {
				$data = array();
				$data = array(
					'pro_id' => $proid,
				...
				if (empty($id)) {
					$success['addtask']  = $this->project_model->Add_Tasks($data);
					...
					$success['addhead']   = $this->project_model->insert_members_Data($data);
					$emid     = $this->input->post('assignto[]');
				   ...
				   $success['addmem']  = $this->project_model->insert_members_Data($data);
					}
					//echo "Successfully Added";
					//print_r($success);
					//if($success){
					...
					$success['update_task'] = $this->project_model->Update_Tasks($id, $data);
	  ...
	  //office
        ...
        if($this->session->userdata('user_login_access') != False) { 
            $id = $this->input->post('id');
            $result_del = $this->project_model->TaskDelete($id);//
          if($result_del){
	...
	public function Add_Field_Tasks()
	{
		if ($this->session->userdata('user_login_access') != False) {
			...
			$this->form_validation->set_rules('details', 'details', 'trim|required|max_length[2024]|xss_clean');
	...
	public function Add_File()
	{
		if ($this->session->userdata('user_login_access') != False) {
			...
			$this->form_validation->set_rules('details', 'details', 'trim|required|max_length[300]|xss_clean');
			...
			if ($this->form_validation->run() == FALSE) {
              ...
              if(!is_dir($config['upload_path'])) mkdir($config['upload_path'], 0777, TRUE);
				...
				if (!$this->upload->do_upload('img_url')) {
					...
					if($success){
						echo json_encode(array('status'=>'success','message'=> 'Project File Successfully Added'));
					}
				}
	...
	public function Delete_Pro_Notes(){
        if($this->session->userdata('user_login_access') != False) { 
            $id = $this->input->post('id');
            $result_del = $this->project_model->Delete_Pro_Notes($id);//
          if($result_del){
	...
	public function Add_Expenses()
	{
		if ($this->session->userdata('user_login_access') != False) {
			...
			$this->form_validation->set_rules('details', 'details', 'trim|required|max_length[250]|xss_clean');
        ...
        if($this->session->userdata('user_login_access') != False) { 
            $id = $this->input->post('id');
            $result_del = $this->project_model->Delete_Expenses($id);//
          if($result_del){
                echo json_encode(array('status'=>'success','message'=> 'Deleted Successfully'));
	...
	public function TasksById()
	{
		if ($this->session->userdata('user_login_access') != False) {
			...
			$data['employesvalue'] = $this->project_model->GetEmployesValue($id);
			$data['teamheadvalue'] = $this->project_model->GetTeamheadValue($id);
			$data['membervalue'] =   $this->project_model->GetTeammemberValue($id);
			//print_r($data['membervalue']);
			echo json_encode($data);
		} else {
			redirect(base_url(), 'refresh');
		}
...
/* 	public function pDelet()
	...
	public function projectDelete(){
        if($this->session->userdata('user_login_access') != False) { 
            $id = $this->input->post('id');
            $result_del = $this->project_model->projectDelete($id);//
          if($result_del){
                  echo json_encode(array('status'=>'success','message'=> 'Deleted Successfully'));
            }else{
                 //echo json_encode(array('status'=>'failed','dep_delfail'=> 'This Department Already used'));
