...
//          var newElement = $('<div class="col-12 mt-2"> <label class="">Annexure ' + i + '</label> <textarea id="content' + i + '" class="mymce content" name="content[]"></textarea> <div class="tag-container" id="tag-container-content' + j + '"></div><button type="button" name="remove" id="' + i + '" class="btn btn-danger btn_remove">Remove</button></div>');
...
//add new
   ...
   // New for tagss single editor only

    // // Function to insert tags into the TinyMCE editor at the current cursor position
    //     function insertTag(tag) {
    //         var editor = tinymce.get('content1'); // Update with your actual TinyMCE editor ID
    //         if (editor) {
    //             editor.selection.setContent(tag);
    //         }
    //     }

    //     // Display tags below the TinyMCE editor
    //     function displayTags(tags) {
    //         var tagsContainer = $('#tag-container'); // Add a container in your HTML to display tags
    //         tagsContainer.empty(); // Clear existing tags

    //         $.each(tags, function (index, tag) {
    //             var tagElement = $('<span class="badge badge-secondary tag" data-tag="' + tag + '">' + tag + '</span>');
    ...
    //     // Sample tags
    //     var sampleTags = ['{first_name}', '{last_name}', '{date}'];

    //     // Initialize tag display
    //     displayTags(sampleTags);
