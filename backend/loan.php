...
<div class="page-wrapper">
    ...
    <div class="row page-titles">
        <div class="col-md-5 align-self-center">
            <h3 class="text-themecolor"><i class="fa fa-hourglass-1" aria-hidden="true"></i> Apply Loan</h3>
        </div>
                ...
                <div class="card card-outline-info">
                    ...
                    <div class="card-body">
                        <div class="table-responsive ">
                            <table id="table1" class="display nowrap table table-hover table-striped table-bordered loan123"
                              ...
                              <?php if( $this->role->User_Permission('apply_loan','can_view') &&  $this->role->User_Permission('apply_loan','can_add') &&  $this->role->User_Permission('apply_loan','can_edit') &&  $this->role->User_Permission('apply_loan','can_delete')){?>
                                ...
                                <?php }elseif($this->role->User_Permission('apply_loan','can_view')){ 
                                ...
                                /*Single user data*/
                                 $id = $this->session->userdata('user_login_id');
                                     $loaninfo = $this->loan_model->emploandata($id);
                                    // $i = count($loaninfo);
                                       if(!empty( $loaninfo)){
                                    ?>
                              
                                     <?php $i = 1;
                                    foreach($loaninfo as $value){ ?>
                                    <tr>
                                        <td><?php echo $i ?></td>
                                        <td><?php echo $value->first_name.' '.$value->last_name ?></td>
                                        <td><?php echo $value->em_code ?></td>
                                        ...
                                        <td class="jsgrid-align-center ">
                                            <!-- Test -->
                                            <?php if($value->hr_status == 'Pending' && $value->status == 'Pending'){ ?>
                                            <a href="#" title="Edit"
                                                class="btn btn-sm btn-info waves-effect waves-light loanmodalclass"
                                                data-id="<?php echo $value->id; ?>"><i
                                                    class="fa fa-pencil-square-o"></i></a>
                                                <?php }elseif ($value->status == 'Pending' && $get_hr_approve) { ?>
                                                <a href="#" title="Edit"
                                                class="btn btn-sm btn-info waves-effect waves-light loanmodalclass"
                                                data-id="<?php echo $value->id; ?>"><i
                                                    class="fa fa-pencil-square-o"></i></a>
                                                  <?php   } ?>
                                             <?php if($value->hr_status == 'Pending' && $value->status == 'Pending'){?>
                                             <button   title="Delete" class="btn btn-sm btn-info waves-effect waves-light noticedel" data-id="<?php echo $value->id; ?>"><i class="fa fa-trash-o"></i></button>
                                             <?php }elseif ($value->status == 'Pending' && $get_hr_approve) { ?>
                                                 <button   title="Delete" class="btn btn-sm btn-info waves-effect waves-light noticedel" data-id="<?php echo $value->id; ?>"><i class="fa fa-trash-o"></i></button>
                                          <?php   } ?>
                        ...
                        <!--  /*Hr dashboard*/ -->
                                ...
                                $id = $this->session->userdata('user_login_id');
                                $hrloanview = $this->loan_model->loan_hr_view($id);
                                ?>
                    <div class="card card-outline-info">
                    <div class="card-header">
                          <h4 class="m-b-0 text-white">&nbsp;&nbsp; Employee Loan List
                        </h4>
                    </div>
                            <div class="card-body">
                                 <div class="table-responsive ">
                            <table id="table2" class="display nowrap table table-hover table-striped table-bordered "
                                cellspacing="0" width="100%">
                                <thead>
                                    <tr>
                                        <th>S.NO</th>
                                     ...
                                     <?php   
                                        ...
                                        if ($get_hr_approve) { ?>
                                         <th>Action </th>
                                       <?php   } ?>
                                           <?php if( $this->role->User_Permission('apply_loan','can_edit') && !$get_hr_approve){?>
                                          <th>Action </th>
                                        <?php } ?>
                                    </tr>
                                </thead>
                                    ...
                                    <tr>
                                        ...
                                        <td class="jsgrid-align-center ">
                                            <a href="#" title="Edit"
                                              ...
                                              <a href="" title="Approve" class="btn btn-sm btn-info waves-effect waves-light hrStatus" data-employeeId="<?php echo $value->em_id; ?>"  data-employeeId="<?php echo $value->id; ?>" data-id="<?php echo $value->id; ?>" data-value1="Granted" >Approve</a>       
                                            <a href="" title="Reject" class="btn btn-sm btn-info waves-effect waves-light  hrStatus"  data-value1="Deny" data-id="<?php echo $value->id; ?>" data-employeeId="<?php echo $value->em_id; ?>"   >Reject</a>
   ...
   $.confirm({
    ...
    $.ajax({
    type: 'post',
    url: '<?php echo base_url('Loan/Loandelete') ?>',
    data: {id:id},
    success: function (response) {
     var data=$.parseJSON(response);
     if(data.status == 'success'){
